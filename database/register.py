def add(db, book_name, **data_to_add):
    response = {}
...
def delete(db, book_name, **with_repr):
    response = {}
    book = db.__dict__[book_name]
...
def update(db, book_name, argument_update, with_repr):
    response = {}
    book = db.__dict__[book_name]
    repr_key, repr_value = '', ''
    for k, v in with_repr.items():
...
def get(db, book_name, with_argument=None):
    response = {}
    book = db.__dict__[book_name]
...
class Manager:
    ...
    for name in os.listdir('./database'):
        if name.endswith('sqlite3'):
            database[name.replace('.sqlite3', '')] = {"path": f'./database/{name}'}

    @staticmethod
    def get_tools():
        return tuple([add, delete, update, get])
